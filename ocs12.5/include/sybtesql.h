/*
**
**	Sybase Precompiler System 12.5.1
**	Confidential Property of Sybase, Inc.
**	(c) Copyright Sybase, Inc. 1995, 2002, 2003
**	All rights reserved
*/
/*
		...
		(_sqlOper == _SQL_CREATE))
	{
		...
		*/
		...
		if (_sqlretcode == CS_SUCCEED)
		{
			...
			/*
			** We are deallocating.
			** In this situation, if we see a failover,
			** we just need to clear the cs_object.
			** We need to propagate CS_RET_HAFAILOVER
			*/
			if (_sqlretcode == CS_RET_HAFAILOVER)
			{
				cs_objects(_sql->ctx, CS_CLEAR,
					&_sql->stmtName, &_sql->stmt);
				_sql->doDecl = CS_TRUE;
			}
	...
	*/
	if (_sqlretcode == CS_SUCCEED && _sqlOper == _SQL_DESTROY) 
	{
		...
		*/
		...
		if (_sqlsavedret != CS_SUCCEED)
		{
			/*
			** Preserve possible CS_FAIL and CS_RET_HAFAILOVER
			*/
			_sqlretcode = _sqlsavedret;
		}
	}
	return(_sqlretcode);
}
